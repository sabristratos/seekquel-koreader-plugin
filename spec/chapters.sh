#!/usr/bin/env bash
#
# Which chapter a position falls in, read off the file's own contents. No server, no
# token, no pairing. Run it while editing.
#
#   spec/chapters.sh
#
...
docker run --rm \
  -v "$MOUNT":/plugin \
  --entrypoint sh \
  "$IMAGE" -c 'exec lua5.1 /plugin/spec/chapters_spec.lua'
